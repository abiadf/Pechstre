...
def _numba_sort_inplace_3d(arr):
    """Blazing fast fast-path sort for edge packing arrays [val, u, v]."""
...
def _flush_h0_numba(h0_edges, parent, birth_val, pix_vals, h0_pairs_out):
    h0_count = 0
    # Pre-fetch array length to optimize loop boundary checks
    n_edges = h0_edges.shape[0]
    for i in range(n_edges):
        ...
        # Inlined path-compressed root discovery
        curr_u = u
        while parent[curr_u] != curr_u:
            parent[curr_u] = parent[parent[curr_u]]
            curr_u = parent[curr_u]
        root_u = curr_u

        curr_v = v
        while parent[curr_v] != curr_v:
            parent[curr_v] = parent[parent[curr_v]]
...
def _flush_h1_numba(h1_edges, parent_f, birth_val_f, face_vals, exterior, h1_pairs_out):
    h1_count = 0
    n_edges = h1_edges.shape[0]
    for i in range(n_edges):
        neg_val = h1_edges[i, 0]
        f1 = int(h1_edges[i, 1])
        f2 = int(h1_edges[i, 2])
        val = -neg_val
        ...
        curr_f1 = f1
        while parent_f[curr_f1] != curr_f1:
            parent_f[curr_f1] = parent_f[parent_f[curr_f1]]
            curr_f1 = parent_f[curr_f1]
        root_f1 = curr_f1

        curr_f2 = f2
        while parent_f[curr_f2] != curr_f2:
            parent_f[curr_f2] = parent_f[parent_f[curr_f2]]
            curr_f2 = parent_f[curr_f2]
        root_f2 = curr_f2
